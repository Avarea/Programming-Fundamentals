...
namespace TravelCompany
{
    class Program
    {
        ...
        static void ProcessCurrentAccomodation(Dictionary<string, int> busesData,string destination,int passengers)
        {
            int availableAccomodation = busesData.Select(kvp => kvp.Value).Sum();
            ...
            {
                Console.WriteLine("{0} -> all except {1} accommodated", destination, passengers - availableAccomodation);
            }
        }
    }
}
